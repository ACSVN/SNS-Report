...
parseDataWithType = function(data_result){
	...
	if(length(data_result) > 0){
		for(i in 1:length(data_result)){
			...
			if(title_tmp[1] == 'F'){
				...
				sum_female_gender = Reduce('+', data_result[[titleType[i]]])
			...
			if(title_tmp[1] == 'M'){
				...
				sum_male_gender = Reduce('+', data_result[[titleType[i]]])
			...
			if(title_tmp[1] == 'U'){
				...
				sum_unkown_gender = Reduce('+', data_result[[titleType[i]]])
	...
	if(length(arr_gender_result) > 0){
		nrow = length(unlist(strsplit(arr_gender_result[[1]][1], ',')))
		ncol = length(titleType)
		arr_gender_result = paste(arr_gender_result, collapse = ',')
	}else{
		nrow = 0
		ncol = 0
		arr_gender_result = list()
	...
	if(length(titleType) > 0){
		...
		# titleType = paste(titleType, collapse = ',')
	}else{
		titleType = c('詳細', '日時')
	}
...
parseData = function(result_by_type, title_des){
	...
	if(length(result_by_type) > 0){
		for(i in 1:length(result_by_type)){
			if(length(result_by_type[[i]]$value) > 0){
				date_result = c(date_result, substr(strftime(result_by_type[[i]]$end_time, "%Y-%m-%d 23:59:59"), 0, 10))
...
parseDataLikeByAgeReachAction = function(dataResult){
